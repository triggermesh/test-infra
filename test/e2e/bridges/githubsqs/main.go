...
var _ = Describe("GitHub to SQS", func() {
	...
	Specify("a GitHub push event is generated by the source and received by the SQS target", func() {
		...
		By("inspecting the message payload", func() {
			...
			Expect(msgData["source"]).To(Equal(repo.GetHTMLURL()))
			Expect(msgData["subject"]).To(ContainSubstring(commit.GetSHA()[:10]))

			eventData, err := json.Marshal(msgData["data"])
			Expect(err).ToNot(HaveOccurred())
			ghEvent := &github.PushEvent{}
			Expect(json.Unmarshal(eventData, ghEvent)).ToNot(HaveOccurred())
